...
/** Wind Turbine menu: no machine slots — upgrade column + player inventory; energy synced. */
public class WindTurbineMenu extends MachineMenu {

    private static final int MACHINE_SLOTS = 0;

    public WindTurbineMenu(int id, Inventory playerInventory) {
        this(id, playerInventory,
                new SimpleContainer(MACHINE_SLOTS + NeroTechMachineBlockEntity.UPGRADE_SLOTS),
                new SimpleContainerData(7));
    }

    public WindTurbineMenu(int id, Inventory playerInventory, Container container, ContainerData data) {
        super(ModMenuTypes.WIND_TURBINE.get(), id, container, data, MACHINE_SLOTS);
