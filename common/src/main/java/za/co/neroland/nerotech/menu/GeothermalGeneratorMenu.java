...
/** Geothermal Generator menu: no machine slots — upgrade column + player inventory; energy/heat synced. */
public class GeothermalGeneratorMenu extends MachineMenu {

    private static final int MACHINE_SLOTS = 0;

    public GeothermalGeneratorMenu(int id, Inventory playerInventory) {
        this(id, playerInventory,
                new SimpleContainer(MACHINE_SLOTS + NeroTechMachineBlockEntity.UPGRADE_SLOTS),
                new SimpleContainerData(7));
    }

    public GeothermalGeneratorMenu(int id, Inventory playerInventory, Container container, ContainerData data) {
        super(ModMenuTypes.GEOTHERMAL_GENERATOR.get(), id, container, data, MACHINE_SLOTS);
