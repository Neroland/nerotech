 ...
 * {@code nerotech:hydrogen} and {@code nerotech:oxygen}.
 *
...
public final class NeroTechFluids {
    ...
    public static Fluid fluidFor(@Nullable Identifier gas) {
        if (NeroTechGases.HYDROGEN.equals(gas)) {
            return HYDROGEN.get();
        }
        if (NeroTechGases.OXYGEN.equals(gas)) {
            return OXYGEN.get();
        }
        return Fluids.EMPTY;
    }
    ...
    public static Identifier gasFor(@Nullable Fluid fluid) {
        return fluid instanceof GasFluid gasFluid ? gasFluid.gas() : null;
    }
